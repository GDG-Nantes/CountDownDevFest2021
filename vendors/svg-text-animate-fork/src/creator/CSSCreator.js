...
export default class CSSCreator extends AnimationCreator {
  ...
  setSVGAnimation() {
    ...
    style.innerHTML = `@keyframes STAdraw{
        0%{
          fill:transparent;
        }
        70%{
  ...
  setPathAnimation(path, i) {
    ...
    switch (_options.mode) {
      case 'sync':
        ...
        break
      case 'delay':
        animation = `animation: STAdraw ${_options['duration']}ms ${_options['timing-function']} ${
          _options.delay * i
        }ms ${_options['fill-mode']} ${_options['direction']} ${_options['iteration-count']};`
        break
      case 'onebyone':
        animation = `animation: STAdraw ${_options['duration']}ms ${_options['timing-function']} ${
          _options['duration'] * i
